...
function RegisterMovie() {
  ...
  let [pageTitle, isEdittingMovie, loadingMessage] = [
    "",
    "",
    "در حال دریافت اطلاعات فیلم",
  ];
  isEdittingMovie = window.location.href.split("?id=")[1];
  useEffect(() => {
    if (isEdittingMovie) {
      setLoading(true);
  ...
  if (!loading) {
    return (
      <div className="container-fluid register-movie py-5">
        ...
        >
          <div className="row">
            <h1>{isEdittingMovie ? "ویرایش فیلم" : "ثبت فیلم/سریال"}</h1>
            ...
            <BorderedButton
              ...
              text={isEdittingMovie ? "ویرایش" : "ثبت"}
              type={""}
              btnType={"submit"}
            />
          </div>
